...
def _note_champ_post_vide(raw_value):
    """True si le champ formulaire est vide (distinct de la note 0)."""
...
def _enregistrer_note_etudiant(user, etudiant, evaluation, *, note_value=None, absent=False, justifie=False):
    ...
    if absent:
        ...
        valider_note_saisie(note_value, evaluation)
        note_obj.absent = False
        note_obj.note = note_value
        note_obj.justifie = False
        note_obj.note_sur = plafond_note_saisie(evaluation)
...
def _enregistrer_notes_etudiants(request, inscriptions, evaluation):
    ...
    for inscription in inscriptions:
        ...
        if _note_champ_post_vide(note_value) and not absent:
            if had_note:
        ...
        note_decimal = None
        if not _note_champ_post_vide(note_value):
            try:
                note_decimal = Decimal(note_value.replace(',', '.'))
            except InvalidOperation as exc:
                raise ValueError(
                    f'{etudiant.nom_complet} : note invalide (« {note_value} »).'
                ) from exc
            valider_note_saisie(note_decimal, evaluation)

        if _enregistrer_note_etudiant(
            request.user,
            etudiant,
            evaluation,
            note_value=note_decimal,
            absent=absent,
            justifie=justifie,
        ):
            enregistrees += 1
...
def note_list(request):
    ...
    return render(request, 'evaluations/note_list.html', {
        ...
        'note_plafond_saisie': (
            plafond_note_saisie(evaluation_obj) if evaluation_obj else None
        ),
    })
...
def saisie_masse_notes(request, evaluation_id):
    ...
    if request.method == 'POST':
        ...
        for inscription in inscriptions:
            ...
            if not _note_champ_post_vide(note_value) or absent:
                ...
                if absent:
                    ...
                    try:
                        ...
                        messages.error(request, f'{etudiant.nom_complet} : note invalide.')
                        return redirect('evaluations:saisie_masse_notes', evaluation_id=evaluation_id)
                    try:
                        valider_note_saisie(note_decimal, evaluation)
                    except ValueError as exc:
                        messages.error(request, f'{etudiant.nom_complet} : {exc}')
                        return redirect('evaluations:saisie_masse_notes', evaluation_id=evaluation_id)
                    note.absent = False
                ...
                note.note_sur = (
                    Decimal(note_sur_value) if note_sur_value
                    else plafond_note_saisie(evaluation)
                )
    ...
    return render(request, 'evaluations/saisie_masse_notes.html', {
        'evaluation': evaluation,
        'etudiants_notes': etudiants_notes,
        'note_plafond_saisie': plafond_note_saisie(evaluation),
    })
    ...
    if request.method == 'POST':
        ...
        if etudiant_id and session_id:
            ...
            if 'enregistrer_notes' in request.POST:
                for evaluation in evaluations:
                    note_value = (request.POST.get(f'note_{evaluation.id}') or '').strip()
                    ...
                    if not _note_champ_post_vide(note_value) or absent:
                        try:
                            ...
                            messages.error(
                                request,
                                f'{evaluation.type_evaluation.nom} ({evaluation.ec.code}) : {exc}',
                            )
        ...
        if ec_id and session_id:
            ...
            if 'enregistrer_notes' in request.POST:
                for inscription in inscriptions:
                    etudiant = inscription.etudiant
                    for evaluation in evaluations:
                        note_value = (request.POST.get(f'note_{etudiant.id}_{evaluation.id}') or '').strip()
                        ...
                        if not _note_champ_post_vide(note_value) or absent:
                            ...
                            if absent:
                                ...
                                note_obj.absent = False
                                note_obj.note = (
                                    Decimal(note_value.replace(',', '.'))
                                    if not _note_champ_post_vide(note_value) else None
                                )
                                note_obj.justifie = False
