...
def deliberation_semestrielle_faite(session, promotion):
    """True si une délibération semestrielle est déjà terminée pour cette promotion."""
...
def promotions_concernees_session(session):
    """
    Promotions ayant des inscrits sur l'année de la session,
    dans une filière qui a des UE sur le semestre de la session.
    """
    from academics.models import Promotion, UniteEnseignement

    annee = session.annee_academique
    filiere_ids = (
        UniteEnseignement.objects.filter(semestre=session.semestre, active=True)
        .exclude(filiere__isnull=True)
        .values_list('filiere_id', flat=True)
        .distinct()
    )
    qs = Promotion.objects.filter(active=True, filiere_id__in=filiere_ids)
    if annee:
        qs = qs.filter(classes__inscriptions__annee_academique=annee).distinct()
    return qs


def actualiser_deliberation_faite_session(session):
    """
    Marque la session comme entièrement délibérée seulement si toutes les
    promotions concernées ont une délibération semestrielle terminée.
    """
    promos = list(promotions_concernees_session(session).values_list('pk', flat=True))
    if not promos:
        ...
        from deliberations.models import Deliberation
            ...
            Deliberation.objects.filter(
                session=session,
                promotion_id__in=promos,
                type_deliberation=Deliberation.TYPE_SEMESTRIELLE,
                statut__in=('terminee', 'verrouillee'),
            ).values_list('promotion_id', flat=True)
        )
        session.deliberation_faite = set(promos) <= done
    session.save(update_fields=['deliberation_faite', 'updated_at'])
    return session.deliberation_faite


def peut_deliberer(session, promotion, *, autoriser_recalcul=False):
    """
    Vérifie les prérequis de délibération semestrielle.
    Retourne (ok: bool, message: str).

    Si autoriser_recalcul=True, une délibération déjà terminée pour la
    promotion n'est plus un blocage (recalcul possible).
    """
    ...
    if deliberation_semestrielle_faite(session, promotion) and not autoriser_recalcul:
        return False, (
            f'La délibération a déjà été effectuée pour la promotion '
            f'{promotion.code} sur cette session.'
        )
    return True, ''
