...
def plafond_note_saisie(evaluation) -> Decimal:
    """Barème maximal pour la saisie manuelle (TJ, examen et rattrapage : /10)."""
    type_evaluation = getattr(evaluation, 'type_evaluation', None)
    if type_evaluation and (
        est_type_tj(type_evaluation) or est_type_exam_ou_rattrapage(type_evaluation)
    ):
        return BAREME_TJ_EXAM
    note_max = evaluation.note_max or BAREME_TJ_EXAM
    return min(note_max, BAREME_TJ_EXAM)


def valider_note_saisie(valeur: Decimal, evaluation) -> None:
    """Lève ValueError si la note est hors barème de saisie."""
    plafond = plafond_note_saisie(evaluation)
    if valeur < 0:
        raise ValueError('La note ne peut pas être négative.')
    if valeur > plafond:
        plafond_affichage = format(plafond, 'f').rstrip('0').rstrip('.')
        raise ValueError(f'La note doit être inférieure ou égale à {plafond_affichage}.')
