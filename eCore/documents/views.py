...
from documents.services import (
    ...
    generer_releve_notes_pdf_bytes,
    generer_releves_notes_zip,
)
    ...
    if all([semestre_id, filiere_id, promotion_id, etudiant_id]):
        ...
        if not annee:
            ...
            pdf_bytes, filename = generer_releve_notes_pdf_bytes(
                etudiant, semestre, filiere, promotion, annee,
            )
            response = HttpResponse(pdf_bytes, content_type='application/pdf')
...
@login_required
def releve_notes_export_bulk(request):
    """Génère une archive ZIP de tous les relevés de notes de la promotion."""
    semestre_id = request.GET.get('semestre')
    filiere_id = request.GET.get('option')
    promotion_id = request.GET.get('promotion')

    if not all([semestre_id, filiere_id, promotion_id]):
        messages.error(request, 'Sélectionnez le semestre, la filière et la promotion.')
        return redirect('documents:releve_notes_selection')
    ...
    filiere = get_object_or_404(Filiere, pk=filiere_id)
    promotion = get_object_or_404(Promotion, pk=promotion_id)
    inscriptions, _ = _inscriptions_pour_releve(promotion_id, filiere_id)

    if not inscriptions.exists():
        messages.warning(request, 'Aucun étudiant inscrit pour cette sélection.')
        return redirect(
            f"{reverse('documents:releve_notes_selection')}"
            f"?semestre={semestre_id}&option={filiere_id}&promotion={promotion_id}"
        )

    zip_bytes, zip_name = generer_releves_notes_zip(
        inscriptions, semestre, filiere, promotion, annee,
    )
