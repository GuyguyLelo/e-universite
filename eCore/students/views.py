...
@login_required
def student_fiche_scolarite(request, pk):
    """Génère la fiche de scolarité PDF de l'étudiant."""
    student = get_object_or_404(Student, pk=pk)
    inscriptions = list(
        student.inscriptions.select_related(
            'annee_academique',
    ...
    from .fiche_scolarite_pdf import build_fiche_scolarite_filename, build_fiche_scolarite_pdf

    pdf_bytes = build_fiche_scolarite_pdf(student, inscriptions)
    filename = build_fiche_scolarite_filename(student)
    response = HttpResponse(pdf_bytes, content_type='application/pdf')
    response['Content-Disposition'] = f'inline; filename="{filename}"'
    return response
